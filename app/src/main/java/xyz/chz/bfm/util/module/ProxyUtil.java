...
public class ProxyUtil {
    ...
    public static boolean isWhiteListMode() {
        String cmd = "grep 'proxy_mode' /data/adb/box/settings.ini | sed 's/^.*=//' | sed 's/\"//g'";
        return "whitelist".equals(MagiskHelper.execRootCmd(cmd));
    }

    public static HashSet<String> getAppidList() {
        HashSet<String> s = new HashSet<>();
        String cmd = "grep 'packages_list' /data/adb/box/settings.ini | sed 's/^.*=//' | sed 's/(//g' | sed 's/)//g' | awk 'END{print}'";
    ...
    public static boolean setAppidList() {
        return MagiskHelper.execRootCmdSilent("sed -i 's/packages_list=.*/packages_list=()/;' /data/adb/box/settings.ini") != -1;
    }
    ...
    public static String getCore() {
        return MagiskHelper.execRootCmd("grep 'bin_name=' /data/adb/box/settings.ini | sed 's/^.*=//g'");
    }

    public static boolean setCore(String x) {
        return MagiskHelper.execRootCmdSilent("sed -i 's/bin_name=.*/bin_name=" + x + "/;' /data/adb/box/settings.ini") != -1;
    }

    public static boolean isWL() {
        return MagiskHelper.execRootCmdSilent("sed -i 's/proxy_mode=.*/proxy_mode=\"whitelist\"/;' /data/adb/box/settings.ini") != -1;
    }

    public static boolean isBL() {
        return MagiskHelper.execRootCmdSilent("sed -i 's/proxy_mode=.*/proxy_mode=\"blacklist\"/;' /data/adb/box/settings.ini") != -1;
    }
    ...
    public static void start(MagiskHelper.Callback callback) {
        new Thread(
                () -> {
                    String cmd = "/data/adb/box/scripts/box.service start && /data/adb/box/scripts/box.iptables enable";
    ...
    public static void stop(MagiskHelper.Callback callback) {
        new Thread(
                () -> {
                    String cmd = "/data/adb/box/scripts/box.iptables disable && /data/adb/box/scripts/box.service stop";
                    MagiskHelper.execRootCmdVoid(cmd, callback);
                }).start();
    }
